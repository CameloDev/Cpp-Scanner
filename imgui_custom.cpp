...
bool m3np::tab(const char* label, bool selected) {
	...
	float textAnim = ImTricks::Animations::FastFloatLerp(std::string(label).append("tab.textAnim"), selected, 1.25f, 1.f, GetIO().DeltaTime * 3);
	float alphaAnim = ImTricks::Animations::FastFloatLerp(std::string(label).append("tab.alphaAnim"), selected, 0.f, 1.f, GetIO().DeltaTime * 3);

	window->DrawList->AddRectFilled({ totalBb.Min.x + 10, window->Pos.y + 61 }, { totalBb.Max.x - 10, window->Pos.y + 65 }, ImColor(123 / 255.f, 123 / 255.f, 233 / 255.f, alphaAnim), 50, ImDrawFlags_RoundCornersTop);
...
bool m3np::tab2(const char* name, bool boolean)
{
...
void m3np::beginChild(const char* name, ImVec2 size) {
	ImGuiWindow* window = GetCurrentWindow();
...
void m3np::renderFrame(ImVec2 start, ImVec2 end, ImColor frameColor, ImColor borderColor, float rounding) {
	PushStyleColor(ImGuiCol_Border, borderColor.Value);
	RenderFrame(start, end, frameColor, true, rounding);
	PopStyleColor();
}

bool m3np::Checkbox(const char* label, bool* v)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return false;
...
void m3np::HelpMarker(const char* desc) {
	ImGui::TextDisabled("(?)");
...
void m3np::Hotkey(int* k, const ImVec2& size_arg) {

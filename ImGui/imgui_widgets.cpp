...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 39.0f, label_size.y + style.FramePadding.y * 6.0f);
    ...
    window->DrawList->AddRectFilled(bb.Min, bb.Max, ImGuiCol_FrameBg, style.FrameRounding);
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(26, 27, 27, int(255 * fill)), style.FrameRounding);
    window->DrawList->AddRect(bb.Min, bb.Max, ImColor(0, 145, 255, int(255 * it_hovered->second)), style.FrameRounding);
...
inline ImColor checkbox_gradient_start = ImColor(0, 145, 255);
inline ImColor checkbox_gradient_end = ImColor(0, 145, 255);

///////////////////////////////////////////// CHECKBOXES

inline ImVec4 checkbox_bg = ImColor(ImGuiCol_FrameBg);
inline ImColor checkbox_mark = ImColor(22, 22, 22);
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    it_anim->second.gradient_color0 = ImLerp(it_anim->second.gradient_color0, *v ? checkbox_gradient_start : hovered ? ImColor(ImGuiCol_FrameBg) : ImColor(ImGuiCol_FrameBg), g.IO.DeltaTime * 6.f);
    it_anim->second.gradient_color1 = ImLerp(it_anim->second.gradient_color1, *v ? checkbox_gradient_end : hovered ? ImColor(124, 25, 255) : ImColor(ImGuiCol_FrameBg), g.IO.DeltaTime * 6.f);
    it_anim->second.checkmark_color = ImLerp(it_anim->second.checkmark_color, *v ? checkbox_mark : hovered ? ImColor(ImGuiCol_FrameBg) : ImColor(ImGuiCol_FrameBg), g.IO.DeltaTime * 39.f);
    it_anim->second.rg = ImLerp(it_anim->second.rg, *v ? 0.f : 10.f, g.IO.DeltaTime * 5.f);


    it_anim->second.text_color = ImLerp(it_anim->second.text_color, *v ? text_active : hovered ? text_hov : text, g.IO.DeltaTime * 6.f);

    const ImRect check_bb(pos, pos + ImVec2(square_sz, square_sz));
    RenderNavHighlight(total_bb, id);

    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32(checkbox_bg), true, style.FrameRounding);
    ...
    window->DrawList->AddRect(check_bb.Min + ImVec2(it_anim->second.rg, it_anim->second.rg), check_bb.Max - ImVec2(it_anim->second.rg, it_anim->second.rg), GetColorU32(it_anim->second.gradient_color0), style.FrameRounding);
    ...
    RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad + 4 * 1, pad + 4 * 1 + it_anim->second.rg), GetColorU32(it_anim->second.checkmark_color), square_sz - pad * 4.0f);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, style.FramePadding.x * 49.0f, (is_multiline ? g.FontSize * 3.0f : label_size.y) + style.FramePadding.y * 4.0f); // Arbitrary default of 8 lines high for multi-line
    const ImVec2 total_size = ImVec2(frame_size.x + (label_size.x > 30.0f ? style.ItemInnerSpacing.x + label_size.x : 30.0f), frame_size.y);
    ...
    it_hovered->second = ImClamp(it_hovered->second + (1.f * ImGui::GetIO().DeltaTime * (hovered || g.ActiveId == id ? 1.f : -1.5f)), 0.0f, 1.0f);
    it_hovered->second *= style.Alpha;
    ...
    {
        RenderNavHighlight(frame_bb, id);
        window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, ImGuiCol_FrameBg, style.FrameRounding);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(26, 27, 27, int(255 * fill)), style.FrameRounding);
        window->DrawList->AddRect(frame_bb.Min, frame_bb.Max, ImColor(0, 145, 255, int(255 * it_hovered->second)), style.FrameRounding);
    }
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_cursor)
        {
            ...
            ImRect cursor_screen_rect(cursor_screen_pos.x, 15 - g.FontSize + 1.5f, cursor_screen_pos.x + 1.0f, cursor_screen_pos.y + 0);
            ...
            {
                filing.insert({ id, {1.f, 1.f} });
                it_filing = filing.find(id);
            }

            if ((frame_bb.Min.x + it_filing->second.value) < cursor_screen_rect.Min.x) {
                it_filing->second.speed = (cursor_screen_rect.Min.x - (frame_bb.Min.x + it_filing->second.value)) / 8.f;
                it_filing->second.value += it_filing->second.speed / ImGui::GetIO().Framerate * 150.f;
            }
            else if ((frame_bb.Min.x + it_filing->second.value) > cursor_screen_rect.Min.x) {
                it_filing->second.speed = ((frame_bb.Min.x + it_filing->second.value - cursor_screen_rect.Min.x)) / 8.f;
                it_filing->second.value -= it_filing->second.speed / ImGui::GetIO().Framerate * 150.f;
            }

            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))

                draw_window->DrawList->AddRectFilled(frame_bb.Min + ImVec2(it_filing->second.value + 0, 4), frame_bb.Min + ImVec2(it_filing->second.value + 1, 25), ImColor(0, 145, 255));
